...
def load_data(db) -> Tuple[np.ndarray, np.ndarray]:
    ...
    try:
        ...
        train_data = list(train_collection.find({}, {"_id": 0, "features": 1, "label": 1}))  # Exclude `_id`
        
        if not train_data:
            raise HTTPException(status_code=404, detail="No training data found in the database")
        
        X_train = np.array([sample["features"] for sample in train_data], dtype=np.float32)
...
def train_model() -> Dict[str, str]:
    ...
    try:
        ...
        return {"status": "success", "message": "Model trained successfully", "code":200}
    except Exception as e:
